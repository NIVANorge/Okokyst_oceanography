...
proj_id <- df_proj %>%
  filter(PROJECT_NAME == "ØKOKYST Nordsjøen 2021-2025") %>%
  pull(PROJECT_ID)
...
if (read_from_nivabase){
  ...
  # saveRDS(df_chem_2021, "Data/17_2021_Nordsjoen.rds")  
  
} else {

  df_chem_2021 <- readRDS("Data/17_2021_Nordsjoen.rds")  
...
## Plots for each parameter {.tabset}  
...
message("NOTE: see bottom for interactive plot of ", var, " with month on x axis")
...
# Note: ggiraph plots will alwys be displayed at the bottom anyway 
gg <- df_chem %>% filter(PARAM_NAME == var) %>%
  ggplot(aes(Month, VALUE, color = STATION_CODE)) +
  geom_line() + 
  geom_point_interactive(aes(tooltip = paste0(STATION_CODE, ": ", VALUE))) + 
...
stations %>% walk(plot_ctdprofile_station, data = dat, 
                  variable = var, points = TRUE, year_by_month = TRUE, 
                  maxdepth = max_depth, maxvalue = 500)
