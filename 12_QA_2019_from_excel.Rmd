...
## 1. Libraries   
After this chunk, you can skip to 11 if you ony want the Norskehavet Sør 1 results  
```{r}
...
# Check those where mn oxygen occurs over several depths
dat_o2min %>% 
  filter(Depth1_min < Depth1_max) %>%
  select(Date, Oksygen, Depth1, Depth1_min, Depth1_max)

dat_oxygen <- bind_rows(
  dat_bottom %>% 
    select(Date, Oksygen, Oksygenmetning, Depth1) %>% 
...
dat_oxygen %>%
  ...
  summarise(Oksygen = mean(Oksygen), 
            Oksygenmetning = mean(Oksygenmetning)) %>%
  knitr::kable(caption = "Mean oxygen for all months")
...
dat_oxygen %>%
  ...
  summarise(Oksygen = mean(Oksygen),
            Oksygenmetning = mean(Oksygenmetning)) %>%
  knitr::kable(caption = "Mean oxygen for winter (Sept - April)")
