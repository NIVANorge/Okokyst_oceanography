...
cat("Summary of data\n")
df_chla2 %>% 
  select(Depth_10_m, Depth_05_m, Abs_difference_10m_vs_05m, Perc_difference_10m_vs_05m) %>%
  summary()
cat("Abs_difference_10m_vs_05m, percentiles\n")
df_chla2$Abs_difference_10m_vs_05m %>% quantile(seq(0,1,0.2), na.rm = TRUE)
cat("Abs_difference_10m_vs_05m, percentiles of absolute values\n")
df_chla2$Abs_difference_10m_vs_05m %>% abs() %>% quantile(seq(0,1,0.2), na.rm = TRUE)
cat("Perc_difference_10m_vs_05m, percentiles\n")
df_chla2$Perc_difference_10m_vs_05m %>% quantile(seq(0,1,0.2), na.rm = TRUE)
cat("Perc_difference_10m_vs_05m, percentiles of absolute values\n")
df_chla2$Perc_difference_10m_vs_05m %>% abs() %>% quantile(seq(0,1,0.2), na.rm = TRUE)
```
...
absdiff_max <- df_chla2$Abs_difference_10m_vs_05m %>% abs() %>% max() %>% ceiling()
absdiff_breaks <- c(0,0.1,0.25,0.5,1,2, absdiff_max)
absdiff_colors <- c("green4","green","orange","orange3","red","red4")
gg <- ggplot(df_chla2, 
             aes(x = Depth_10_m, 
                 y = Depth_05_m, 
                 color = cut(abs(Abs_difference_10m_vs_05m), 
...
gg <- ggplot(df_chla2, aes(Time, Abs_difference_10m_vs_05m)) +
...
#### Full x and y scales
```{r, fig.height=7, fig.width=9, echo=FALSE, warning=FALSE}
percent_max <- df_chla2$Perc_difference_10m_vs_05m %>% abs() %>% max() %>% ceiling()
percent_breaks <- c(0,10,20,50,100,300, percent_max)
percent_colors <- c("green3","green","orange","orange3","red","red4")
gg <- ggplot(df_chla2, aes(Depth_10_m, Depth_05_m, 
                     color = cut(abs(Perc_difference_10m_vs_05m), percent_breaks, include.lowest = TRUE))
       ) +
  geom_point() +
...
#### Full y scale
```{r, fig.width=9, echo=FALSE, warning=FALSE}
ggplot(df_chla2, aes(Time, Perc_difference_10m_vs_05m)) +
  geom_line() +
  facet_wrap(~StationCodeName)
```
...
plot_scatter <- function(codename){
  df_chla2_sel <- df_chla2 %>% 
    filter(StationCodeName == codename & !is.na(Abs_difference_10m_vs_05m))
  absdiff_max <- df_chla2_sel$Abs_difference_10m_vs_05m %>% 
    abs() %>% max() %>% ceiling()
  if (absdiff_max > 2){
    absdiff_breaks <- c(0,0.1,0.25,0.5,1,2, absdiff_max)
    absdiff_colors <- c("green4","green","orange","orange3","red","red4")
  ...
  gg <- ggplot(
    ...
    coord_fixed() +
    labs(title = codename, x = "Chl a, 10 m", y = "Chl a, 5 m")
  gg
}

save_scatter <- function(codename, subfolder){
  gg <- plot_scatter(codename)
  ggsave(sprintf("Figures_10m_vs_5m/%s/%s Scatter.png", subfolder, codename), gg, width = 5, height = 4, dpi = 500)
}
...
plot_diff <- function(codename){
  ...
  labs(title = codename, x = "Time", y = "Chl a difference (10 m minus 5 m)")
  gg
  }
...
save_diff <- function(codename, subfolder){
  ...
  ggsave(sprintf("Figures_10m_vs_5m/%s/%s Diff.png", subfolder, codename), gg,     width = 5, height = 4, dpi = 500)
}
...
plot_time <- function(codename){
  ...
  geom_line(color = "black") +
  geom_point(size = 3) +
  geom_point(data = df_chla2_sel, aes(Time, Depth_10_m), color = "blue", shape = 4, size = 3) +
  scale_color_manual("Absolute\ndifference", values = absdiff_colors, na.value = "grey30") +
  scale_y_continuous(breaks = seq(0, 20, 1)) +
  geom_hline(yintercept = c(3.5,7), linetype = "dashed") +
  expand_limits(y = 0) +
...
save_time <- function(codename, subfolder){
  ...
  ggsave(sprintf("Figures_10m_vs_5m/%s/%s Time.png", subfolder, codename), gg,     width = 5, height = 4, dpi = 500)
}
