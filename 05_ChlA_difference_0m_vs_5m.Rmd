...
__If `save_plots` is set to true, these plots are saved to file__
```{r}
...
plot_scatter <- function(codename){
  ...
  gg <- ggplot(
    ...
    coord_fixed() +
    labs(title = codename, x = "Chl a, 5 m", y = "Chl a, 0 m")
  gg
}
...
plot_stack <- function(codename){
  ...
  gg <- ggplot(df, 
    aes(x = Depth, y = KlfA, color = Second_highest)) +
    geom_point(shape = "_", size = 9) +
    geom_hline(yintercept = c(3.5,7), linetype = "dashed") +
