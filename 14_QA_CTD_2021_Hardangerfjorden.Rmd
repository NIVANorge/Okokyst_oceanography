---
title: "QA for ØKOKYST Nordsjøen 2021 - Hardangerfjorden CTDs "
author: "DHJ"
date: "13 1 2020"
output:
  html_document:
...
- Stations:    
    * VR48, VR49, VT53, VT70, VT74, VT8 - Hardangerfjorden (this script)  
...
dir("C:/Users/DHJ/NIVA/Trond Kristiansen - Nordsjoen_Nord")
dir("C:/Users/DHJ/NIVA/Trond Kristiansen - Nordsjoen_Nord/xlsfiles/Hardangerfjorden/")
...
if (read_excel_files){
  ...
  datafolder <- "C:/Users/DHJ/NIVA/Trond Kristiansen - Nordsjoen_Nord/xlsfiles/Hardangerfjorden/"
  ...
  saveRDS(dat, "Data/14_QA_CTD_2021_Hardangerfjorden.rds")
  
} else {
  
  dat <- readRDS("Data/14_QA_CTD_2021_Hardangerfjorden.rds")
...
dat_old2 <- readRDS("Data/14_QA_CTD_2020_Hardangerfjorden.rds")
...
### CHANGE STATION of one station/date   
* VR49 of 2021-10-19 should be VT70  
* See demo / comments in 'Oxygen volume part'   
```{r, echo = FALSE, fig.width=9, fig.height=7, warning=FALSE}

dat_orig <- dat
sel <- dat$StationCode == "VR49" & dat$Date == ymd("2021-10-19"); sum(sel)
dat$StationCode[sel] <- "VT70"
...
```{r}
df <- dat %>%
  filter(StationCode == "VT74" & Month == 3)
...
- VR49: The measurement 19-10-2021 must be from another station? Probably VT70
    - CTD taken two days in a row in VR49?  
    - Huge change in bottom oxygen from one day to the next? (and back again afterwards)
    - No CTD taken in VT70 in October  
    - October values fits VT70 values  
...
stations %>% walk(
  plot_ctdprofile_station,
  data = dat_orig, variable = "Oksygen", points = FALSE)

```

#### Amended data    
* VR49 of 19. Oct changed to VT70    
* The data set used in all other plots    
```{r, echo = FALSE, fig.width=9, fig.height=7, warning=FALSE}
...
if (FALSE){
  # check depths
  dat2 %>%
    filter(StationCode %in% "VR49") %>% # View()
    group_by(Date) %>%
...
plot_timeseries_station(stationcode = "VR49", data = dat2, variable = "Saltholdighet", titletext = "", Depth_is_depth1 = TRUE)
plot_timeseries_station(stationcode = "VR49", data = dat2, variable = "Temperatur", titletext = "", Depth_is_depth1 = TRUE)
plot_timeseries_station(stationcode = "VR49", data = dat2, variable = "Oksygen", titletext = "", Depth_is_depth1 = TRUE)
plot_timeseries_station(stationcode = "VR49", data = dat2, variable = "Oksygenmetning", titletext = "", Depth_is_depth1 = TRUE)
